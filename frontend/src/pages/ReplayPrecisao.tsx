  ...
  return <div className="col" style={{ gap: 18 }}>
    ...
    {dados && atual && <>
      ...
      <Card style={{ padding: 18 }}>
        ...
        <div className="col" style={{ gap: 8, marginTop: 14 }}>{dados.examples.map((item) => <button key={item.event_id} onClick={() => setEvidencia(item)} className="row gap2" style={{ width: "100%", textAlign: "left", border: "1px solid var(--line)", background: "#fff", borderRadius: 10, padding: "11px 12px", flexWrap: "wrap" }}><div className="grow" style={{ minWidth: 220 }}><b style={{ fontSize: 12.5 }}>{item.group}</b><div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 3 }}>{item.day.split("-").reverse().join("/")} · {nomeHumano(item.label)} · {item.start_s.toFixed(0)}–{item.end_s.toFixed(0)}s</div></div><span style={{ fontSize: 11.5, color: "var(--muted)" }}>R1</span><Decisao valor={item.r1} /><Icon name="arrow-right" size={14} color="var(--muted)" /><Decisao valor={item.current} /><Icon name="images" size={16} color="var(--accent)" /></button>)}</div>
      </Card>
