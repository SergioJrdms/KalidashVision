...
import type { IndicadorEvidencia } from "../lib/types";
...
type Props = {
  ...
  janelaDias?: number;
  /** Alias legado; novos indicadores devem usar indicador + janelaDias. */
  janelaPresenca?: number;
  onClose: () => void;
};

function duracao(s: number) { return s >= 60 ? `${Math.round(s / 60)} min` : `${Math.round(s)} s`; }

const evidenceModalStyles = `
  @keyframes evidence-backdrop-in {
    from { opacity: 0; }
    to { opacity: 1; }
  }

  @keyframes evidence-modal-in {
    from { opacity: 0; transform: translateY(9px) scale(.98); }
    to { opacity: 1; transform: translateY(0) scale(1); }
  }

  .event-evidence-backdrop {
    position: fixed;
    inset: 0;
    z-index: 70;
    display: flex;
    ...
    animation: evidence-backdrop-in 220ms ease-out both;
  }
  ...
  .event-evidence-modal {
    ...
    animation: evidence-modal-in 240ms cubic-bezier(.22, 1, .36, 1) both;
  }

  @media (max-width: 640px) {
    .event-evidence-backdrop { padding: 8px; }
    .event-evidence-modal {
  ...
  @media (prefers-reduced-motion: reduce) {
    .event-evidence-backdrop,
    .event-evidence-modal { animation-duration: 1ms; }
  }
`;

/** Rótulo de auditoria: apenas traduz o estado já resolvido pelo backend. */
export function rotuloLeituraPresenca(estadoPresenca?: string | null) {
  ...
  if (estadoPresenca === "posto_vazio") return "Sem operador no posto";
  return "Sem operador no posto";
}

export function rotuloLeituraIndicador(indicador?: IndicadorEvidencia | null, estadoPresenca?: string | null) {
  if (indicador === "produtivo") return "Produtivo";
  if (indicador === "improdutivo") return "Improdutivo";
  if (indicador === "sem_decisao") return "Sem decisão";
  if (indicador === "presenca_operador") return "Presença do operador";
  if (indicador === "posto_sem_operador") return "Sem operador no posto";
  if (indicador === "presenca_inconclusiva") return "Sem leitura de presença";
  if (indicador === "posto_vazio") return rotuloLeituraPresenca(estadoPresenca);
  if (indicador === "tempo_capturado") {
    if (estadoPresenca === "produtivo" || estadoPresenca === "operador_fora_produtivo") return "Produtivo";
    if (estadoPresenca === "improdutivo" || estadoPresenca === "operador_fora_improdutivo") return "Improdutivo";
    return "Sem decisão";
  }
  return rotuloLeituraPresenca(estadoPresenca);
}

/** Painel reutilizável: lista leve primeiro; os frames só são requisitados ao abrir um evento. */
export function EventEvidenceDrawer({ processoId, labels, titulo, categoria, indicador, janelaDias, janelaPresenca, onClose }: Props) {
  const [aberto, setAberto] = useState<string | null>(null);
  const janelaIndicador = janelaDias ?? janelaPresenca;
...
function EvidenceFrames({ id, pessoa, label, ini, fim, segundoAngulo }: { id: string; pessoa: number; label: string; ini: number; fim: number; segundoAngulo?: { segmento_id: string; cam_id: string | null; offset_s?: number } | null }) {
  ...
  const intervaloRecuperado = cam1.data?.origem_frames === "intervalo_correspondente"
    ? cam1.data.intervalo_frames
    : null;
